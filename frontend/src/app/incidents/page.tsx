...
export default async function IncidentsPage() {
  ...
  return (
    <div>
      ...
      <SearchableList
        placeholder="Search incident title or summary…"
        emptyMessage="No incident scenarios match."
        resultsClassName="relative space-y-5 before:content-[''] before:absolute before:left-[7px] before:top-2 before:bottom-2 before:w-px before:bg-[var(--sd-border)]"
        items={scenarios.map((scenario) => ({
          id: scenario.id,
          severity: scenario.severity,
          searchText: `${scenario.title} ${scenario.summary}`,
          content: (
